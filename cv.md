---
title: Curriculum Vitae
layout: default
use_fontawesome: true
---

<div class="container">
    <div class="row">
        <div class="d-none d-md-block col-md-7 col-lg-7 col-xl-7"> <!-- desktop: hide on screens smaller than md) -->
            <iframe src="https://docs.google.com/gview?url=gatesdupont.github.io/attachments/DupontCV.pdf&embedded=true" style="width:100%; height:100%;" frameborder="0">
            </iframe>
        </div>
        <div class="d-md-none col-12 col-sm-12" style="margin-bottom: 10%;"> <!--  mobile: hide on screens larger than sm -->
            <iframe src="https://docs.google.com/gview?url=gatesdupont.github.io/attachments/DupontCV.pdf&embedded=true" style="width:100%; height:80vh;" frameborder="0">
            </iframe>
        </div>
        <div class="col-12 col-sm-5 col-md-5 col-lg-5 col-xl-5">
            <img width="100%" height="100%" src="{{ site.baseurl }}/images/alaska.jpeg">
        </div>
    </div>
</div>
